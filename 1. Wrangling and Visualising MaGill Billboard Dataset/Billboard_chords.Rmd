...
<p>We can get a better sense of just how popular some of these chord progressions are if we plot them as a percentage of the top 12 most common chord progressions on a bar chart. </p>
...
bb_bigram_perc$bigram <- gsub(":maj", "maj  ", bb_bigram_perc$bigram) 
  ...
  ggplot(bb_bigram_perc, aes(x = reorder(bigram, n), y = share, fill = n)) +
  geom_col() +
  coord_flip() +
  scale_y_continuous(labels = percent_format(accuracy = 1), expand = c(0, 0), breaks = pretty_breaks(n = 10)) +
  ...
  theme(legend.position = "none",
        ...
        plot.title = element_text(family = "Source Sans Pro", size = 16, color = "white", face = "bold", hjust = 0.6,
                                  margin = margin(b = 10, unit = "pt")),
  ...
  geom_label(aes(label = paste0(bigram, "   ", round(share*100, 1), "%")), fill = "black", 
             position = position_stack(0), hjust = -0.01, label.padding  = unit(0.41, "lines"),
             family = "Source Sans Pro", size = 7, color = "white", fontface = "bold") #+
  #geom_text(aes(label = paste0(round(share*100, 1), "%")), position = position_stack(1), hjust = 1,
   #         family = "Source Sans Pro", size = 6, color = "black", face = "bold")
