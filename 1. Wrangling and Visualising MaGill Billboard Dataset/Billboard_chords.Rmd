...
<p>We can get a better sense of just how popular some of these chord progressions are if we plot them as a percentage of the top 20 most common chord progressions on a bar graph. Note how the most common chord change, G major to D major, occurs more than twice as often than even some of the other top 20 chord bigrams.</p>
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.align = "center", fig.width = 6.8, fig.asp = 1.14}
 ...
 bb_bigram_count %>%
  slice(1:20) %>%
  mutate(share = n/sum(n)) %>%
  ggplot(aes(x = reorder(bigram, n), y = share, fill = n)) +
  geom_col() +
  coord_flip() +
  scale_y_continuous(labels = percent_format(accuracy = 1), expand = c(0, 0), breaks = pretty_breaks(n = 9)) +
  scale_fill_gradient(low = "#0095CC", high = "#CC0044") +
  #xlab("Chord changes") +
  #ylab("% from 20 songs") +
  theme_minimal() +
  theme(legend.position = "none",
        ...
        #axis.line.x = element_line(size = 1, color = "red"),
        ...
        plot.title = element_text(family = "Source Sans Pro", size = 16, color = "white", face = "bold", hjust = 0.9,
                                  margin = margin(b = 10, unit = "pt")),
        plot.subtitle = element_text(family = "Source Sans Pro", size = 14, color = "white", face = "bold", hjust = 0.5,
                                     margin = margin(b = 10, unit = "pt")),
        plot.caption = element_text(family = "Source Sans Pro", size = 10, color = "white",
                                    margin = margin(t = 20, unit = "pt"))) +
  labs(title = "Percentage of the top 20 most common chord progressions",
       subtitle = "in the McGill Billboard Dataset", 
       caption = "Topic: Kris Shaffer, DataCamp project | 
                       Data: McGill Billboard Dataset | ggplot2 chart: M. Blum-Oeste") +
  geom_text(aes(label = paste0(bigram, "   ")), position = position_stack(0), hjust = 0,
            family = "Source Sans Pro", size = 6, color = "white", face = "bold") +
  geom_text(aes(label = paste0(round(share*100, 1), "%")), position = position_stack(1), hjust = 1,
            family = "Source Sans Pro", size = 6, color = "white", face = "bold")
